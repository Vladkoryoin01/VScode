...
getAllServicePrices()
// 2️⃣ Функция getFullPrice (Function Declaration)  
// Возвращает полную стоимость проекта (услуги + вёрстка)
function getFullPrice(allServicePrices, screenPrice) {
    return allServicePrices + screenPrice;
}

// 3️⃣ Функция getTitle  
// Изменяет регистр названия проекта (первый символ - заглавный, остальные - строчные)
function getTitle(titleProject) {
    return titleProject.charAt(0).toUpperCase() + titleProject.slice(1).toLowerCase();
}

// 4️⃣ Функция getServicePercentPrices  
// Вычитает 10% комиссии подрядчику
function getServicePercentPrices(fullPrice) {
    return Math.round(fullPrice * 0.9);
}

// 5️⃣ Функция getRollbackMessage  
// Вычисляет скидку в зависимости от стоимости проекта
